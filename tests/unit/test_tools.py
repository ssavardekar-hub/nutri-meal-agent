...
def test_manage_pantry_view_add_remove_clear(tmp_path, monkeypatch):
...
def test_search_recipes_api_mock_fallback():
    # Test query filter
    recipes_spinach = search_recipes_api(query="spinach")
    ...
    # Test cuisine filter
    recipes_med = search_recipes_api(cuisine="Mediterranean")
    assert len(recipes_med) > 0
    assert all(r["cuisine"].lower() == "mediterranean" for r in recipes_med)


def test_get_daily_meal_plan(tmp_path, monkeypatch):
    test_pantry_file = tmp_path / "pantry.json"
    test_profile_file = tmp_path / "profile.json"
    monkeypatch.setattr("app.tools.PANTRY_FILE", test_pantry_file)
    monkeypatch.setattr("app.tools.PROFILE_FILE", test_profile_file)

    manage_pantry(action="add", items=["spinach", "eggs", "olive oil"])
    manage_profile(action="update", medical_conditions=["diabetes", "hypertension"])

    plan = get_daily_meal_plan(day_preference="today")
...
def test_export_grocery_list(tmp_path, monkeypatch):
    test_pantry_file = tmp_path / "pantry.json"
    test_profile_file = tmp_path / "profile.json"
    monkeypatch.setattr("app.tools.PANTRY_FILE", test_pantry_file)
    monkeypatch.setattr("app.tools.PROFILE_FILE", test_profile_file)
    ...
    grocery = export_grocery_list(items=items_to_buy)
...
def test_calculate_nutrition_targets():
...
def test_swap_meal(tmp_path, monkeypatch):
    test_pantry_file = tmp_path / "pantry.json"
    test_profile_file = tmp_path / "profile.json"
    monkeypatch.setattr("app.tools.PANTRY_FILE", test_pantry_file)
    monkeypatch.setattr("app.tools.PROFILE_FILE", test_profile_file)

    swapped = swap_meal(meal_type="breakfast", current_recipe_id=101, reason="taste preference")
